...
class MapCreateError(Exception):
    ...
    MINE_INDEX_INVAID = 'invalid mine index'
...
class Map(object):
    """
    A map consist of some mine's position data. It is read-only so that you its attributes after initing.
    Attributes:
        ...
        mine_list: A list, every element is a two_tuple like (x,y) stand for a mine's position.
        mine_nuber: A integer less or equal than map_size, the amount of mine
    ...
    def __init__(self, height, width, mine_number=None, mine_index_list=None, mine_pos_list=None):
        """
        Create a map with mines. You can provide mine position in following three ways.
        (Order by priority)
        :param height:
        :param width:
        :param mine_number:
        :param mine_index_list:
        :param mine_pos_list:
        """
        ...
        if mine_pos_list:
            ...
            self._creat_mine_list(mine_index_list)
        else:
    ...
    def _create_mine_list(self, mine_index_list):
        ...
        for index in index_set:
            if index in xrange(0, self.map_size):
                ...
                raise MapCreateError(MapCreateError.MINE_INDEX_INVAID)
    ...
    def _generate_distribute_map(self):
        ...
        for t_x, t_y in self.mine_list:
            self.distribute_map[t_x][t_y] = Map.MINE_FLAG
    ...
    def is_in_map(self, pos, offset=None):
        """
        return the given position is in the map.
        :param pos:
        :param offset:
        :return:
        """
...
class Game(object):
    """
    A state machine for playing minesweeper. There are some attribute and actions as usual machine.
    Const attributes(will not change when running):
    mine_map:The map object the game is playing on. As convenience, the game extend map attributes as
    its own attributes. For example, Game.height is for short to Game.mine_map.height
    Runtime attributes:
    ...
    def move(self, click_pos):
        ...
        if near_mine_number == Map.MINE_FLAG:
            ...
            if self._invisual_number == self.mine_map.mine_nuember:
                self._state = Game.STATE_SUCCESS
            else:
                self._state = Game.STATE_PLAY
            return self._state
    ...
    def _complete_game(self):
        ...
        self._invisual_number = self.mine_map.map_size - self.mine_map.mine_nuember
